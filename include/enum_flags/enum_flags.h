/**
 * A type-safe bit flags for C++11 scoped enumerations.
 */
...
template <typename Enum>
    requires std::is_scoped_enum_v<std::decay_t<Enum>>
             && std::is_unsigned_v<std::underlying_type_t<std::decay_t<Enum>>>
class EnumFlags {
...
public:
    ...
    constexpr void swap(EnumFlags& flags) noexcept {
        using std::swap;
        swap(flags_, flags.flags_);
    }

    constexpr bool operator==(const EnumFlags& flags) const noexcept {
        return flags_ == static_cast<RawType>(flags);
    }
